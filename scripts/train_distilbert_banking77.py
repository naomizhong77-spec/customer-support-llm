"""Train Baseline 2 (BANKING77): DistilBERT classification head for 77-intent classification.

This is the BANKING77 counterpart to scripts/train_distilbert.py. The Bitext
script is frozen for reproducibility, so this file is a deliberate minimal-change
copy: only data paths, column names, num_labels, artifact filenames, and the
test-set hash procedure differ. Hyperparameters are held constant (max_length,
batch size, learning rate, warmup, bf16) except num_train_epochs bumped 3 -> 5
to give the 77-class task more room (EarlyStoppingCallback(patience=2) will cut
short if val macro-F1 plateaus).

Differences vs train_distilbert.py:
  ...
  - test_set_hash is the FULL sha256 of the parquet FILE bytes, matching the
    BANKING77 cross-model contract documented in scripts/train_tfidf_banking77.py.
  - Output locations carry the `_banking77` suffix:
      checkpoints/distilbert_banking77/<run_id>/
      outputs/metrics/distilbert_banking77_<YYYYMMDD>_<shorthash>/results.json
      outputs/figures/09_distilbert_banking77_confusion_matrix.png
      outputs/error_analysis/distilbert_banking77_errors.csv

Usage:
    python scripts/train_distilbert_banking77.py \
        --config configs/distilbert_banking77_config.yaml
    python scripts/train_distilbert_banking77.py \
        --config configs/distilbert_banking77_config.yaml --smoke-test

Smoke-test mode uses 100 train / 50 val / 50 test samples, max 50 steps, and
writes to a separate run_id prefixed "distilbert_banking77_smoke_". Respects
--output-root so artifacts can go to /tmp and never pollute real runs.
"""
...
# Logging
# ---------------------------------------------------------------------------
logger = logging.getLogger("train_distilbert_banking77")
...
def test_set_hash(parquet_path: Path) -> str:
    ...
    Expected pin (config-driven): see configs/distilbert_banking77_config.yaml.
    """
    with open(parquet_path, "rb") as f:
        digest = hashlib.sha256(f.read()).hexdigest()
    return f"sha256:{digest}"
...
def plot_confusion_matrix(
    ...
    labels: list[str],
    save_path: Path,
    title: str = "DistilBERT — BANKING77 Intent Confusion Matrix (normalized)",
) -> None:
...
def run_pipeline(  # noqa: C901, PLR0912, PLR0915
    ...
    for p in (train_path, val_path, test_path):
        if not p.exists():
            raise FileNotFoundError(
                f"Missing data split: {p}. Regenerate via the BANKING77 cleaning "
                "notebook (notebooks/08_*.ipynb)."
            )

    # Run identity
    model_tag = "distilbert_banking77_smoke" if smoke_test else "distilbert_banking77"
    ...
    # Verify test-set hash pin if provided (fail fast if the test split drifted)
    observed_hash = test_set_hash(test_path)
    expected_hash = runtime_cfg.get("expected_test_set_hash")
    ...
    # Label space: in smoke mode we may not see all 77 intents in the 100 train
    ...
    # Detach EarlyStoppingCallback before post-training evaluate/predict calls.
    # Those calls use metric_key_prefix="val"/"test", so the still-attached
    # callback (which was built to watch "eval_macro_f1") would fire a spurious
    # "metric not found, early stopping is disabled" warning on every call.
    # Training-time early stopping is already done; the callback has no job left.
    ...
    # Test metrics (use the full 77-intent label order so matrix is comparable)
    ...
    results: dict[str, Any] = {
        "run_id": run_id,
        "model_name": runtime_cfg.get("model_name", "DistilBERT (BANKING77)"),
        ...
        "parse_errors": 0,
        "notes": (
            f"Baseline 2 — BANKING77 (DistilBERT). epochs={epochs} max_steps={max_steps} "
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train DistilBERT intent classifier (Baseline 2, BANKING77)."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/distilbert_banking77_config.yaml"),
